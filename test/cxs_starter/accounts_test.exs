defmodule CxsStarter.AccountsTest do
  ...
  describe "register_user/1" do
    ...
    test "validates maximum values for email and password for security" do
      ...
      assert "should be at most 180 character(s)" in errors_on(changeset).password
    end

    test "validates email uniqueness" do
      %{email: email} = user_factory()
  ...
  describe "deliver_update_email_instructions/3" do
    ...
    test "sends token through notification", %{user: user} do
      ...
      text_token = extract_user_token(email.text_body)

      assert html_token == text_token

      {:ok, token} = Base.url_decode64(html_token, padding: false)
  ...
  describe "update_user_email/2" do
    setup do
      ...
      %{user: user, token: html_token, email: new_user_email}
    end
  ...
  describe "change_user_name/2" do
    ...
    test "allows fields to be set" do
      changeset =
        Accounts.change_user_name(%User{}, %{
          "name" => "New name"
        })
  ...
  describe "update_user_name/3" do
    ...
    test "requires name to change", %{user: user} do
      {:error, changeset} = Accounts.update_user_name(user, valid_user_password(), %{})
    ...
    test "validates maximum value for name for security", %{user: user} do
      ...
      {:error, changeset} =
        Accounts.update_user_name(user, valid_user_password(), %{name: too_long})
    ...
    test "validates current password", %{user: user} do
      {:error, changeset} =
        Accounts.update_user_name(user, "invalid", %{name: unique_user_name()})
  ...
  describe "deliver_user_confirmation_instructions/2" do
    ...
    test "sends token through notification", %{user: user} do
      ...
      email = Accounts.deliver_user_confirmation_instructions(user, CxsStarter.MockUrlProvider)
  ...
  describe "deliver_user_reset_password_instructions/2" do
    ...
    test "sends token through notification", %{user: user} do
      CxsStarter.MockUrlProvider
      |> expect(:reset_password_url, fn token ->
        "[TOKEN]#{token}[TOKEN]"
      end)
      |> expect(:base_url, fn ->
        "base_url"
      ...
      email = Accounts.deliver_user_reset_password_instructions(user, CxsStarter.MockUrlProvider)
  ...
  describe "change_delete_account/2" do
    test "returns a user changeset" do
      assert %Ecto.Changeset{} = Accounts.change_delete_account(%User{})
    end

    test "allows fields to be set" do
      changeset = Accounts.change_delete_account(%User{}, %{})

      assert is_nil(get_change(changeset, :hashed_password))
    end
  end

  describe "delete_account/3" do
    setup do
      %{user: user_factory()}
    end

    test "validates current password", %{user: user} do
      {:error, changeset} = Accounts.delete_account(user, "invalid", %{})

      assert %{current_password: ["is not valid"]} = errors_on(changeset)
    end

    test "deletes user account", %{user: user} do
      {:ok, _deleted_user} = Accounts.delete_account(user, valid_user_password(), %{})
